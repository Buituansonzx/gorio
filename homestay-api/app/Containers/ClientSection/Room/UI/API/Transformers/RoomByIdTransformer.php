...
//        'weekdayPrices',
    ...
    public function transform(Room $room): array
    {
        ...
        return [
            ...
            'features' => [
                [
                    'key'=> 'checkin_checkout_type',
                    'label'=>$room->checkinMethods->first()->getTranslation('name', $language),
                    'description'=>$room->checkinMethods->first()->getTranslation('description', $language),
                ],
                [
                    'key' => 'is_by_hour',
                    'label' =>$room->hourlyPricing->isNotEmpty() ? 'Cho phép đặt theo giờ' : null,
                    'description' => "Homestay cho phép đặt phòng theo giờ"
                ],
                [
                    'key'=> 'fixed_time_std',
                    'label'=>$room->fixedCheckTimeStd->first()?->getTranslation('name', $language),
                    'description'=>$room->fixedCheckTimeStd->first()?->getTranslation('description', $language),
                ],
            ],
            'hourly_pricing' => [
        ...
        if ($room->medias && $room->medias->isNotEmpty()) {
            $imagesArray = $room->medias->map(function ($media) {
                return [
                    ...
                    'image_url' => app(ImageService::class)->toResponsivePayload($media,'(max-width: 768px) 100vw, 960px','content-1440')['sources'],
                    'sort_index' => $media->sort_index,
                    'room_image_area_group_code' => $media->imageAreaGroup?->code,
                    'room_image_area_group_name' => $media->imageAreaGroup?->getTranslation('name', request()->header('Accept-Language')),
                ];
            })->toArray();
    ...
    public function includeHighlightAmenities(Room $room)
    {
        if ($room->highlightAmenities && $room->highlightAmenities->isNotEmpty()) {
            $highlightAmenitiesArray = $room->highlightAmenities->map(function ($roomHighlightAmenity) {
                $images = [];
                if (!empty($roomHighlightAmenity->images)) {
                    $images = $roomHighlightAmenity->images->map(function ($media) {
                        return [
                            'id' => $media->id,
                            'highlight_amenity_id' => $media->highlight_amenity_id,
                            'image_url' => app(ImageService::class)->toResponsivePayload($media,'(max-width: 768px) 100vw, 960px','content-1440')['sources'],
                        ];
                    });
                }
